...
- **Look** - the same design system as the Android app (Bloom): warm cream ground, terracotta for anything you act on, sage as the second accent, soft rounded cards and pill buttons, in the platform's own sans-serif. Dark mode walks the same hues down a neutral ramp. Tokens live at the top of `css/style.css`; the social layout (sidebar, feed, posts, rail, Explore, profile, post page) is in `css/social.css`.
