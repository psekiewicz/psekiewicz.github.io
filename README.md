...
- **Look** - ink on warm paper with a single vermilion accent, hairline rules instead of drop shadows, tight corners instead of pills, and small-caps micro-labels doing the work headings used to. The palette before this was indigo→cyan gradients on cool grey with soft-shadowed rounded cards - the default look of every dashboard template, and an odd fit under a monospace face. Colour now marks things you can act on; everything else is set in type. Shop cosmetics are deliberately exempt: those gradients are what people bought.
