...
- **Quick create sheet** — on mobile the + tab no longer navigates to the dashboard: it rotates 45° into a close control and slides a create sheet up from the bottom (`js/create-sheet.js`), stopping short of the tab bar so the + stays visible and can close it again. The sheet carries the essentials with a link out to the full editor. Only local modules are imported statically — anything touching Supabase is pulled in on submit, so opening and filling the sheet never waits on the network.
