...
- **Customizable Scrolls page** — Scrolls shows a project full-screen and portrait, where a wide thumbnail crops badly, so a project can carry its own Scrolls image (`projects.scroll_image_url`) or pick a gradient (`scroll_bg`, constrained in `schema.sql`) to be text-only instead. Offered in both the quick-create sheet and the full dashboard editor, with the same live preview in each; `js/scroll-styles.js` is the single source of the preset list so the two can't drift.
...
js/scroll-styles.js      Scrolls appearance presets shared by the sheet, the dashboard editor and the feed
