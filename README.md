...
- **XP & levels** — every account has a level, shown as a "Lv 7" chip next to their name on their profile, in the navbar, on project cards, in the Scrolls feed, and on every comment; the profile also gets a progress bar to the next level. **XP is earned only from other people**: distinct signed-in people who opened your entries (2 XP each), likes received (30), comments other people left you (15) and followers (60). Nothing you can do to yourself counts — publishing pays nothing on its own, your own comments pay nothing, self-likes are rejected by policy, and reloading your own entry doesn't register a view. The weights live in the `user_reputation` view in `schema.sql`, not in JS: the browser can't be the authority on what a like is worth, and one definition is what stops the level on a card disagreeing with the level on the profile. `js/levels.js` only turns that number into a level, on a quadratic curve (level *L* starts at 100·(*L*−1)² XP) so early levels come quickly and later ones stay meaningful. Levelling a whole page of authors is a single query against the view — it used to be four, plus a fifth for likes.
